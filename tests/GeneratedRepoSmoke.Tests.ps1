Describe 'Generated repository scaffold' {
    ...
    It 'has the reference-mode workflow entry points' {
        foreach ($relativePath in @(
            ...
            Test-Path -LiteralPath (Join-Path -Path $script:RepoRoot -ChildPath $relativePath) -PathType Leaf |
                Should -BeTrue -Because "$relativePath should exist"
        }
    }

    It 'does not vendor ai-repo-workflow implementation files' {
        Test-Path -LiteralPath (Join-Path -Path $script:RepoRoot -ChildPath 'tools/ai-repo-workflow') |
            Should -BeFalse -Because 'reference-mode repos should use config/ai-repo-workflow.reference.jsonc'
    }
}
